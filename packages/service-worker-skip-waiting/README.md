...
Браузер читает `sw.js` -> происходит инициализация сервис воркера: выполняется описанная в `sw.js` логика, создаются объекты, открывается соединение с IndexedDB и т.п.   
...
1. При первой установке браузер:
    1. Проведет первичную инициализацию sw.
    2. Вызовет событие `install`. Если обработка события `install` не выбросила ошибку, тогда далее сработает событие `activate` и при его успешном завершении sw перейдет в активное состояние, начнет слушать события: `fetch`, `message`, `push`, `sync` и др., и как-то на них реагировать.
2. Перед каждой попыткой инициализировать sw браузер проверяет: "А не изменилось ли содержимое `sw.js`?". Если изменилось, тогда браузер запустит процесс установки нового sw (обновление sw на новую версию):
    1. Проведет инициализацию текущей версии sw (ранее установленной).
    2. Вызовет событие `install`, если обработка события `install` не выбросила ошибку, тогда новый sw переходит в режим ожидания до тех пор, пока текущий активный sw контроллирует хотя бы одного клиента. Как только таких клиентов не останется будет вызвано событие `activate` и после его успешного завершения новый sw перейдет в активное состояние.
