...
2. Вызовет событие `install`. Если обработка события `install` не выбросила ошибку, тогда новый sw переходит в режим ожидания до тех пор, пока текущая версия sw контролирует хотя бы одного клиента. Как только таких клиентов не останется, будет вызвано событие `activate` и после его успешного завершения новый sw перейдет в активное состояние.
...
Повторная попытка выполняется согласно алгоритму "2. Обновление" (см. выше). При этом дополнительно вносить изменения в файл `sw.js` на сервере необязательно(разве что ошибку в логике исправить, если она там есть), т.к. браузер его не установил и при следующей попытке он для него будет считаться измененным.
