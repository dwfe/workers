...
Для таких сайтов HTTP cache не способен на 100% решить проблему актуальности статики, которая может быть запрошена с сервера только один раз при открытии сайта, при рефреше страницы или при смене роута.
...
Несколько кейсов для обычного режима (проверено на хроме):
...
Следом за `sw.js` браузер также выкачивает все файлы скриптов из `importScripts`.
...
1. Проведет инициализацию текущей(ранее установленной) версии sw, если текущая версия sw еще не проинициализирована.
...
Когда новый sw переходит в активное состояние, то клиенты о нем еще ничего не знают.  
...
Повторная попытка выполняется согласно алгоритму "2. Обновление" (см. выше). При этом дополнительно вносить изменения в файл `sw.js` на сервере не обязательно(разве что ошибку в логике исправить, если она там есть), т.к. браузер его не установил и при следующей попытке он для него будет считаться измененным.
