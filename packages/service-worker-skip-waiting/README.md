...
Через 30 секунд неактивности sw хром может перевести его в статус `STOPPED`.  
...
### После рефреша страницы sw не переинициализируется

Sw сам по себе вообще никак не реагирует на обновление страницы(F5). То есть он даже инициализацию самого себя не производит.  
Просто сидит проинициализированный в памяти и ждет работу.
