...
просто потому, что для успешной работы со всем функционалом сайта это не требуется.  
+Весь роутинг может происходить внутри страницы, без прямого обращения к серверу.

Для таких сайтов HTTP cache не способен на 100% решить проблему обновления статики, которая может быть запрошена с сервера только один раз при открытии сайта, при рефреше страницы или при смене роута.
...
- последовательно выполняется описанная в `sw.js` логика;
- создаются объекты;
- открывается соединение с IndexedDB и т.п.
...
Несколько кейсов, связанных с инициализацией sw (при условии, что файл `sw.js` на сервере остается неизменным):
...
- есть мнение, что один раз в 24 часа хром предпринимает самостоятельную попытку скачать `sw.js` с сервера, но надеяться на это не стоит.

При этом браузер следом за `sw.js` также выкачивает все файлы скриптов из `importScripts`.

После каждого скачивания браузер проверяет: "А не изменилось ли содержимое `sw.js`? А не изменилось ли содержимое какого-либо файла из `importScripts`?".  
...
Если выполнить команду `self.skipWaiting()`, тогда браузер пропустит этап ожидания и после успешно завершенного события `install` сразу вызовет событие `activate`.

### Активного состояния недостаточно

Когда новый sw переходит в активное состояние, клиенты о нем еще не знают.  
Это значит, например, что в событие `fetch` не будут прилетать сетевые запросы.  
...
Повторить установку возможно согласно алгоритму "2. Обновление" (см. выше). При этом дополнительно вносить изменения в файл `sw.js` на сервере не обязательно(разве что ошибку в логике исправить, если она там есть), т.к. браузер его не установил и при следующей попытке он для него будет считаться измененным.
...
Поэтому в обработку события `activate` следует добавлять действия, которые не могут быть критичными при нормальном режиме работы нового sw.
