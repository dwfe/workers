...
1. При первой установке браузер вызывает событие `install`. Если обработка события `install` не выбросила ошибку, тогда далее сработает событие `activate` и при его успешном завершении sw перейдет в активное состояние, начнет слушать события: `fetch`, `message`, `push`, `sync` и др., и как-то на них реагировать.
2. Перед каждой инициализацией sw браузер проверяет: "А не изменилось ли содержимое `sw.js`?". Если изменилось, тогда браузер запустит процесс установки нового sw: вызовет событие `install`, если обработка события `install` не выбросила ошибку, тогда новый sw переходит в режим ожидания до тех пор, пока текущий активный sw контроллирует хотя бы одного клиента. Как только таких клиентов не останется будет вызвано событие `activate` и после его успешного завершения новый sw перейдет в активное состояние.
