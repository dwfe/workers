# Режимы работы сервис воркера(sw)

### Нормальный режим

Браузер перечитывает `sw.js` -> происходит инициализация сервис воркера: выполняется описанная в `sw.js` логика, создаются объекты, открывается соединение с IndexedDB и т.п.   
Если в sw отсутствет активность, тогда браузер может его остановить. Интервалы проверки на активность зависят от браузера, например, в хроме это ~30 секунд.  
Если sw остановлен, а от него требуется выполнить работу, тогда браузер **заново** инициализирует sw.  
И вот такой цикл: Инициализация -> Останов -> Инициализация -> Останов -> Инициализация -> ... - является нормальным режимом работы sw.
  
Рефреш страницы сам по себе никакого влияния на sw не производит, то есть, если обновить страницу, то переинциализации sw не произойдет.  
...
Если браузер остановил sw, когда страница не закрыта(допустим пользователь сейчас работает с другим сайтом), то активация закладки со страницей с остановленным sw по умолчанию не инициализирует sw. 

### Установка -> Ожидание -> Активация

1. При первой установке браузер вызывает событие `install`. Если при обработке события `install` не возникло ошибок, тогда сразу сработает событие `activate` и при его успешном завершении sw перейдет в активное состояние и начнет свою обычную работу - слушать события: `fetch`, `message`, `push`, `sync` и др. - и как-то на них реагировать.
2. Перед каждой инициализацией sw браузер проверяет: "А не изменилось ли содержимое `sw.js`?". Если изменилось, тогда браузер начнет установку нового sw: вызовет событие `install`, если при обработке `install` не произошло ошибок, тогда новый sw переходит в режим ожидания до тех пор, пока текущий активный sw контроллирует хотя бы один клиент. Как только таких клиентов не останется будет вызвано событие `activate` и после его успешного завершения новый sw перейдет в активное состояние.
3. Если при обработке событий `install`, `activate` произошла ошибка, тогда браузер попытается снова запустить установку при следующем рефреше страницы.

# Особенности

### В одном окне событие 'fetch' всегда вызывается только на одном зарегистрированном sw
